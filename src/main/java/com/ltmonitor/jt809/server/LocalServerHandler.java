...
public class LocalServerHandler extends IoHandlerAdapter {
	...
	public void exceptionCaught(IoSession session, Throwable e)
			throws Exception {
		this.logger.error("从链路通讯时发生异常：" + e);
	}

	public void messageReceived(IoSession session, Object message)
			throws Exception {
		com.ltmonitor.jt809.server.LocalServer.session = session;
		...
		if (mm.getMsgType() != 0x9005 
				&& mm.getMsgType() != 0x1006
	...
	public void sessionClosed(IoSession session) throws Exception {
		T809Manager.subLinkConnected = false;//从链路断开
		session.close(true);
		this.logger.error("上级平台与本地服务器断开从连接：" + session.getId());
