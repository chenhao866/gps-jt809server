...
public class DownExgMsgReportDriverInfo implements IReceiveProtocol {
	...
	public String handle(JT809Message message) {
		...
		if (StringUtil.isNullOrEmpty(message.getPlateNo()) == false) {
			//DriverModel dm = this.getDriverInfo(message.getPlateNo());
			DriverModel dm = GlobalConfig.driverMap.get(message.getPlateNo());
	...
	private DriverModel getDriverInfo(String plateNo) {
		...
		VehicleData vd = (VehicleData) ServiceLauncher.getVehicleDataService().find(hql,
				plateNo);
		...
		DriverInfo dr = (DriverInfo) ServiceLauncher.getDriverInfoService().find(hql,
				vd.getEntityId());
		if (dr != null) {
