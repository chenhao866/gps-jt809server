...
public class UpBaseMsgVehicleAddedAck implements ISendProtocol {
	...
	private String getCarInfo(VehicleModel vm) {
		...
		sb.append("VIN:=").append(vm.getPlateNo()).append(";VEHICLE_COLOR:=")
				.append(vm.getPlateColor()).append(";VEHICLE_TYPE:=")
				.append(vm.getVehicleType())
				.append(";TRANS_TYPE:=")
				.append(vm.getTransType())
				// 运输行业编码
				// .append(";VEHICLE_NATIONALITY:=").append(vm.getNationallity())
				// //行政区规划代码
				.append(";VEHICLE_NATIONALITY:=").append(vm.getNationallity())
				// 行政区规划代码
				.append(";OWERS_ID:=").append(vm.getOwnerId())
				// 车辆业主信息(不是平台业主信息)
